...
static int _patch_mov_mem_and_reg(MOV_TYPE type, char *address_to_patch,
    ...
    if (operand_size == 128) {
        TRACER_PRINT_ERROR("Tried to patch_mov_mem_and_reg with 128 bytes");
    }
#endif
    switch (type) {
    case MEM_TO_REG: {
   ...
   byte");*/
        ...
        TRACER_PRINT_DEBUG_COLLECTOR("Patching mov %i, [%i]", dest_index,
                                     source_index);

        break;
    }
    case REG_TO_MEM: {
        TRACER_PRINT_DEBUG_COLLECTOR("Patching mov [%i], %i", source_index,
                                     dest_index);
    ...
    if (operand_size == 256) {
        ...
        TRACER_PRINT_DEBUG_COLLECTOR("Operand size is %i", operand_size);
        if (operand_size == 64) {
    ...
    if (type == MEM_TO_REG) {
        if (instruction->info.raw.disp.size > 0) {
            if (instruction->info.raw.disp.size == 8 &&
                ...
                if (is_already_modrm_01 != 1) {
                    address_to_patch[modrm_index] += 0x80;
                } else {
                    address_to_patch[modrm_index] += 0x40;
                }
                *((int *)(&address_to_patch[modrm_index + 1 + has_sib_byte])) =
                    instruction->info.raw.disp.value;
		has_displacement = 4;
                /*TRACER_PRINT_ERROR("mem_to_reg, disp size 32 is not handled, "
                                   "instruction is %s",
                                   instruction->text);
				   */
            }
...
patch_pre_not_written_into_reg(char *address_to_patch,
    ...
    if (get_value_size(instruction) <= 8) {
        ...
        TracerRegister address_reg = address_reg_to_tracer_reg(address_r);
    ...
    int size = _patch_mov_mem_and_reg(
        ...
        get_value_size(instruction) <= 8 ? 64 : get_value_size(instruction) * 8,
...
patch_default_collect_read_value(char *address_to_patch,
    ...
    for (int i = 0; i < instruction->info.operand_count; i++) {
        if (instruction->operands[i].type == ZYDIS_OPERAND_TYPE_REGISTER) {
            ...
            found_reg = 1;
#endif
            reg = get_offset_of_reg(instruction->operands[i].reg.value);
            TRACER_PRINT_DEBUG("got reg index first, reg is %x\n", reg);
            if (reg >= IGNORE) {
                reg = get_offset_of_xmm_reg(instruction->operands[i].reg.value);
                TRACER_PRINT_DEBUG("got reg index second, reg is %x\n", reg);
            }
        }
    }
...
get_register_for_trace_value_address(ZydisDisassembledInstruction *instruction,
    ...
    if (used & R15) {
        if (!(used & R14)) {
            TRACER_PRINT_DEBUG_COLLECTOR("Using R14 for the value address");
            return ADDRESS_R14;
        }
    }
    if (used & R14) {
        if (!(used & R13)) {
            TRACER_PRINT_DEBUG_COLLECTOR("Using R13 for the value address");
            return ADDRESS_R13;
        }
    }
    if (used & R13) {
        if (!(used & R12)) {
            TRACER_PRINT_DEBUG_COLLECTOR("Using R12 for the value address");
            return ADDRESS_R12;
        }
    }
...
int patch_collect_instructions(unsigned long address_to_patch,
    ...
    for (int i = 0; i < length; i++) {
        TRACER_PRINT_DEBUG("%02hhx ", ((char *)address_of_instruction)[i]);
    }
    TRACER_PRINT_DEBUG("\n");
#endif

    if (instruction->info.mnemonic == ZYDIS_MNEMONIC_MOVSB ||
    ...
    if (!is_write(instruction)) {
        ...
        case ZYDIS_MNEMONIC_VPCMPEQB: {
            ...
            int size = patch_pre_not_written_into_reg(
                (char *)address_to_patch + offset, instruction, reg);
            ...
            ZyanStatus status = ZydisDisassembleIntel(
                ZYDIS_MACHINE_MODE_LONG_64, address_to_patch + offset,
                (void *)(address_to_patch + offset), size, &instr);
...
*(long *)((long)(&instr) + i);
        ...
        */
            offset += size;
            TRACER_PRINT_DEBUG("before patch collect read value, instr is %s\n",
                               instr.text);
    ...
    if (needs_value_copy(instruction)) {
        ...
        if (instruction->info.attributes & ZYDIS_ATTRIB_IS_RELATIVE) {
            offset += patch_mov_relative_to_addr_in_reg(
                (char *)address_to_patch, 64, instruction, reg, &used);
    ...
    if (instruction->info.attributes & ZYDIS_ATTRIB_HAS_REP) {
        if (instruction->info.mnemonic == ZYDIS_MNEMONIC_STOSB) {
            ...
            offset += patch_rep_prefix((char *)address_to_patch, instruction);
        }
    }
    ...
    if (instruction->info.opcode_map == ZYDIS_OPCODE_MAP_0F) {
        ...
        case 0xC3: // movnti
            ...
            if (instruction->info.opcode == 0xB1) {
                ...
                if (!for_trampoline) {
                    offset += load_rax((char *)address_to_patch + offset);
                }
            }
            break;
        ...
        case 0xd6: // vmovq
        {
            int res = set_mem_address_to_reg_address_vex(
                instruction, address_of_instruction,
                (char *)address_to_patch + offset, value_reg, NULL_F);
#ifdef TRACER_LOG_DEBUG_COLLECTOR
            length = res;
#endif
            offset += res;
...
#endif
            TRACER_PRINT_ERROR("Collection of two byte opcode %hhx not "
        ...
        case 0xaa:
        case 0xab:
            break; // stosb and stosq, is handled below or above somewhere
        ...
        case 0x83: {
            ...
            default: {
                ...
                info->implemented = TRACER_STATE_NOT_IMPLEMENTED;
#endif
                TRACER_PRINT_ERROR("Collection of opcode %hhx with extension "
                                   "%i not implemented",
...
int patch_mov_dest_addr_value_to_addr_in_reg(
    ...
    int size = _patch_mov_mem_and_reg(
        MEM_TO_REG, address_to_patch, op_size, temp_reg,
        get_offset_of_reg(source->mem.base), instruction);
    size +=
        _patch_mov_mem_and_reg(REG_TO_MEM, address_to_patch + size, op_size,
                               temp_reg, address_reg_to_tracer_reg(dest), NULL);
    return size;
}
...
int set_mem_address_to_reg_address(ZydisDisassembledInstruction *instruction,
    ...
    int opcode_length =
        instruction->info.raw.modrm.offset - instruction->info.raw.prefix_count;
    ...
    if (tracer_reg_to_index(reg) % 8 != 5) {
        ...
        new_instruction[total_length] = 0x20 + (tracer_reg_to_index(reg) % 8);
        total_length += 1;
    } else {
        // we use the modrm_byte with iwth mod 01
        new_instruction[total_length] = 0x40 + (tracer_reg_to_index(reg) % 8) +
...
int set_mem_address_to_reg_address_vex(
    ...
    if (get_value_size(instruction) == 4 || get_value_size(instruction) == 8) {
    ...
    if (tracer_reg_to_index(reg) % 8 != 5) {
        // set modrm/byte to always use the sib byte but keep the register
        address_to_patch[offset] = 0x04 + instruction->info.raw.modrm.reg * 8;

        offset += 1;
        // add the sib byte to point to the correct register
        address_to_patch[offset] = 0x20 + (tracer_reg_to_index(reg) % 8);
        offset += 1;
    } else {
        // we use the modrm_byte with iwth mod 01
        address_to_patch[offset] = 0x40 + (tracer_reg_to_index(reg) % 8) +
                                   instruction->info.raw.modrm.reg * 8;
        // set displacement to 0;
    ...
    for (int i = 0; i < instruction->info.raw.imm[0].size / 8; i++) {
        address_to_patch[offset] =
            instruction_address[instruction->info.raw.imm[0].offset + i];
        offset += 1;
    }
...
long get_and_set_value_address(ZydisDisassembledInstruction *instruction,
    ...
    retry: {
        ...
        if (needs_allignment(instruction)) {
            ...
            TRACER_PRINT_DEBUG_COLLECTOR("Allining with offset %i", offset);
        ...
        if (!__atomic_compare_exchange_8(&buffer->next_offset, &old_offset,
...
static int _patch_mov_value_at_rel_address_to_reg(
    ...
    TracerRegister temp_reg) {
    int size = 0;
    size += patch_mov_absolute_offset_to_reg(instruction, temp_reg, address);
    // now, the address is in temp register, now we do mov temp_reg,
    // [temp_reg]

    size += _patch_mov_value_at_address_to_same_reg(temp_reg, address + size);
    return size;
};
int patch_mov_relative_to_addr_in_reg(char *address_to_patch, int op_size,
    ...
    size += _patch_mov_value_at_rel_address_to_reg(instruction,
                                                   address_to_patch, temp_reg);
    size +=
        _patch_mov_mem_and_reg(REG_TO_MEM, address_to_patch + size, op_size,
                               temp_reg, address_reg_to_tracer_reg(dest), NULL);
    return size;
